...
class Options(str, Enum):
    ...
    pose = 'Pose'
@router.post('/perform_different_modes')
async def perform_different_modes(check : Options,file: UploadFile = File(...), user: dict = Depends(current_user), db: Session = Depends(get_db)):
    return await perform_model(db, user, check, file)